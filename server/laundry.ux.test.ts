...
import { customerStatusSteps, getCustomerNextStep, getCustomerTrackingPath } from "../client/src/lib/laundry";
...
describe("customer tracking UX", () => {
  ...
  it("sends visitors to login and customers to their order space", () => {
    expect(getCustomerTrackingPath(false)).toBe("/connexion");
    expect(getCustomerTrackingPath(true)).toBe("/mon-espace");
  });
});
